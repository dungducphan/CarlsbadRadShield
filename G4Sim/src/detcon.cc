...
G4VPhysicalVolume *detcon::Construct() {
    ...
    // Dimensions
    NumberOf3inHDCShieldingLayersIn12inHDC = 4;
    ...
    double outerShieldingWidth_18inHDC = innerShieldingWidth_18inHDC + 2 * NumberOf3inHDCShieldingLayersIn12inHDC * thicknessOfShieldingLayer_3inHDC;
    ...
    for (unsigned int i = 0; i < NumberOf3inHDCShieldingLayersIn12inHDC; i++) {
...
G4VIStore* detcon::CreateImportanceStore() {
    ...
    istore->AddImportanceGeometryCell(256, *vec_physical_volumes[7]); // Rest of the world
    G4cout << "Going to assign importance: " << 256 << ", to volume: " << (vec_physical_volumes[7])->GetName() << G4endl;

    for (int j = 0; j < vec_physical_volumes_phantom.size(); j++) {
        istore->AddImportanceGeometryCell(256, *vec_physical_volumes_phantom[j], j); // Element Phantoms
        G4cout << "Going to assign importance: " << 256 << ", to volume: " << vec_physical_volumes_phantom[j]->GetName() << G4endl;
    }
